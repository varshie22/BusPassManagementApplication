...
public class StopDAO implements DAO<Stop> {
    ...
    public int insert(Stop object) {
        String sql = "INSERT INTO Stop (address, sequenceOrder, routeId, adminId) " +
                "VALUES ('"+object.address+"', "+object.sequenceOrder+", "+object.routeId+", "+object.adminId+")";
        return db.executeSQL(sql);
    }
    @Override
    public int update(Stop object) {
        String sql = "UPDATE Stop set address = '"+object.address+"', sequenceOrder = "+object.sequenceOrder+", " +
                "routeId = "+object.routeId+" WHERE id = "+object.id;
        return db.executeSQL(sql);
    }
    @Override
