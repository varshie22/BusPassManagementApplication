...
public class UserDAO implements DAO<User>{
    ...
    public int insert(User object) {
        String sql= "INSERT INTO User (name, phone, email, password , address, department, type)"
                + "VALUES('"+object.name+"', '"+object.phone+"', '"+object.email+"', "
                + "'"+object.password+"', '"+object.address+"', '"+object.department+"', '"+object.type+"')";
        return db.executeSQL(sql);
    }
    @Override
    public int update(User object) {
        String sql = "UPDATE User set name='"+object.name+"', phone='"+object.phone+"', password='"+object.password+"', " +
                "address='"+object.address+"', department='"+object.department+"' WHERE email ='"+object.email+"'" ;
        return db.executeSQL(sql);
    }
    @Override
